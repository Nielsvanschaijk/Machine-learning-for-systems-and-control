...
class UnbalancedDisk(gym.Env):
    def __init__(self, nvec=40, umax=3., dt=0.025, render_mode='human'):
        ...
        self.reward_fun = lambda self: (
            ...
            + 1000000 if abs(self.th) > 3.05 else 0
            # + 1000 * np.exp(- (self.th - np.pi)**2 / (2 * 0.5**2))
            + 10000 * np.exp(- (self.th - np.pi)**2 / (2 * 0.1**2)) # was 500 / 750 bij 0.3
            + 1000 * np.exp(- (self.th - np.pi)**2 / (2 * 0.3**2)) # wAS 10000
            + 100 * np.exp(- (self.th - np.pi)**2 / (2 * 0.6**2))
            + 0.5 * (1 - np.cos(self.th)) * abs(self.omega) # was 0.5
            + 0.5 * np.exp(- (abs(self.th) - np.pi/2)**2 / (2 * 0.2**2)) * abs(self.omega) # high velocity 90 degrees
            # + 10 * np.exp(- (self.th - np.pi)**2 / (2 * 1**2))
            + 20 * np.cos(self.th - np.pi) # height was 20
            - 10 * np.exp(- (self.th + np.pi)**2 / (2 * 0.2**2)) if abs(self.th) < 3.0 and abs(self.th) > 0.5 * np.pi else 0 # punish -2/3pi
            # + 3 * abs(self.omega)
