...
class UnbalancedDisk(gym.Env):
    def __init__(self, nvec=40, umax=3., dt=0.025, render_mode='human'):
        ...
        self.previous_omega = 0  # Initialize the previous_omega attribute
        
        self.reset()

    def reward_function(self, th, omega, dt=0.025):
        # Calculate reward
        reward = 0.1 * omega**2 if abs(th) < 3 else 1/omega**2 if omega != 0 else 0
        # - 0.001 * self.u**2

        # Additional punishment for persistent spinning: if the pendulum has been spinning for more than a certain time window, apply a larger penalty
        if self.previous_omega and np.sign(self.previous_omega) == np.sign(omega):
            reward -= 5000  # A larger penalty for persistent direction spinning

        # Store omega for the next time step
    ...
    def step(self, action):
        ...
        # Directly use the reward_function method here
        reward = self.reward_function(self.th, self.omega)
    ...
    def get_obs(self):
        # Add noise to theta (simulated encoder noise)
        scale_th_noise = 0.001
        th_noise = self.th + np.random.normal(loc=0, scale=scale_th_noise)

        # If first time, initialize previous theta
        if not hasattr(self, "prev_th_noise"):
            self.prev_th_noise = th_noise

        # Compute numerical derivative (omega_estimate)
        omega_estimate = (th_noise - self.prev_th_noise) / self.dt

        # Save for next time
        self.prev_th_noise = th_noise

        # Return observation: theta noisy + omega_estimate
        return np.array([th_noise, omega_estimate])
...
# PPO Training Loop
def train_ppo(envs, total_steps=1000):
    returns_history = []
    theta_history = []
    omega_history = []
    reward_history = []
...
def evaluate_policy(env, render=False):
    ...
    while True:
        ...
        total_reward += reward
        time.sleep(0.125)  # Slow down rendering
        if render:
            env.render()
...
if __name__ == "__main__":
    ...
    # Train PPO agent
    train_ppo(envs, total_steps=2000)

    # Plot the reward function heatmap after training is complete
    plot_reward_heatmap()

    # Evaluate trained agent
    env_eval = UnbalancedDisk(dt=0.025)
    env_eval = gym.wrappers.TimeLimit(env_eval, max_episode_steps=250)
