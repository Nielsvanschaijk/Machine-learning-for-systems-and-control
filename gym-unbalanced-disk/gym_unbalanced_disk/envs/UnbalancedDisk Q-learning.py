...
class UnbalancedDisk(gym.Env):
    ...
    def __init__(self,nvec=40, umax=3., dt = 0.025, render_mode='human'):
        ...
        # self.reward_fun = lambda self: np.exp(abs(self.th)) - 0.01 * self.delta_th**2
        # self.reward_fun = lambda self: (self.th)**2 - 0.001 * self.delta_th**2 - 0.001 * self.omega**2
        # self.reward_fun = lambda self: 1 if abs(self.th) > 3 else 0#np.exp(abs(self.th))#.5 * (1 - np.cos(self.th))#- ((self.th - np.pi)**2 + 0.1 * self.omega**2 + 0.01 * self.u**2)
        # self.reward_fun = lambda self: 3 if abs(self.th) > 3 and self.delta_th < 0.15 else \
                                        # 2 if abs(self.th) > 3 else \
                                        # 1 if abs(self.th) > 0.7 else \
                                        # 0.5 if self.delta_th > 0.15 else 0
        # self.reward_fun = lambda self: np.exp(abs(self.th)) - 0.01 * self.delta_th**2
        # self.reward_fun = lambda self: -1 * abs(self.th)
        # self.reward_fun = lambda self: 10000 if abs(self.th) > 3 and self.delta_th < 5 else \
        #                                 100 if abs(self.th) > 3 else \
        #                                 5 * self.delta_th if self.delta_th >= 0 else \
        #                                 0
        # self.reward_fun = lambda self: 10000 if abs(self.th) > 3 and self.delta_th < 5 else \
        #                                 100 if abs(self.th) > 3 else \
        #                                 5 * np.abs(self.delta_th) # gaat naar 650
        # # self.reward_fun = lambda self: 10000 if abs(self.th%(2*np.pi)) > 3 and self.delta_th < 5 else \
        #                                 100 if abs(self.th%(2*np.pi)) > 3 else \
        #                                 10 * np.abs(self.delta_th) + 10 * abs(self.th%(2*np.pi))# if abs(self.th%(2*np.pi)) > 1 else \
        #                                 # 5 * np.abs(self.delta_th)
        self.reward_fun = lambda self: 10000 if self.costh > 0.9 and self.delta_th < 2 else \
                                        100 - 5 * np.abs(self.delta_th) if self.costh > 0.9  else \
                                        5 * np.abs(self.delta_th) + 5 * self.costh # gaat naar 650
    ...
    def step(self, action):
        ...
        terminated = abs(self.th) > 3 and abs(self.delta_th) < 0.1# > 0.9 and abs(self.omega) < 1
...
def train():
    ...
    for nvec in [10]:#, 20]:#, 40, 80, ]: #c) # was 5,10,20,40,80
        ...
        print('nvec=',nvec) #c)
        Qmat, ep_lengths_steps, ep_lengths, info = Qlearn(env, nsteps=500_000, callbackfeq=5000) #c=) # was 400_000
        rewards = info[0]
        omegas = info[1]
        actions = info[2]
        thetas = info[3]
...
def run_simulation():
    ...
    env = Discretize_obs(env, nvec=100) # was 100
    Qmat = Qmats[40]
