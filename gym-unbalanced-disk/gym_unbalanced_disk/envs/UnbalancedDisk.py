...
class UnbalancedDisk(gym.Env):
    ...
    def __init__(self, umax=3.0, dt=0.025, render_mode="human"):
        ...
        self.reward_fun = lambda self: np.exp(
            -((self.th % (2 * np.pi) - np.pi) ** 2) / (2 * (np.pi / 7) ** 2)
        )  # example reward function, change this!
...
class UnbalancedDisk_sincos(UnbalancedDisk):
    ...
    def get_obs(self):
        ...
        return np.array(
            [np.sin(self.th_noise), np.cos(self.th_noise), self.omega_noise]
