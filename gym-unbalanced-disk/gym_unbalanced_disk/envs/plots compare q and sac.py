...
class UnbalancedDiskSAC(gym.Env):
    def __init__(self,nvec=40, umax=3., dt = 0.025, render_mode='human'):
        ############# start do not edit  ################
        self.omega0 = 11.339846957335382
        self.delta_th = 0
...
class UnbalancedDisk_sincos(UnbalancedDiskSAC):
...
def plots_moving():
    ...
    for _ in range(499):  # or however long you want to run
        ...
        # demo_env.render()
        anglesSAC.append(info['ths'] * -1)
        actionsSAC.append(action * -1)
    ...
    plt.plot(timesteps, anglesQ, label="Q-learning")
    plt.plot(timesteps, anglesSAC, label='SAC')
    plt.legend()
    ...
    plt.plot(timesteps, actionsQ, label = 'Q-learning')
    plt.plot(timesteps, actionsSAC, label = 'SAC')
    plt.legend()
    plt.xlabel("Time step")
    plt.ylabel("input velocity")
    plt.title("input velocity $u$ over time")
    plt.show()
