...
def train_actor_critic(env, nvec=10, Actor=None, Critic=None):
    import os
    if Actor is None:
        if os.path.exists('actor_policy.npy'):
            print("Loading saved Actor...")
            Actor = np.load('actor_policy.npy')
        else:
            Actor = np.zeros((nvec, nvec, env.action_space.n))
    
    if Critic is None:
        if os.path.exists('critic_values.npy'):
            print("Loading saved Critic...")
            Critic = np.load('critic_values.npy')
        else:
            Critic = np.zeros((nvec, nvec))
    ...
    for i in tqdm(range(100_000)):
        ...
        advantage = (returns - Critic[obs_start])

        tmp = np.zeros((env.action_space.n,))
        tmp[action] = 1
        grad_actor = (tmp - probs) * advantage
        grad_critic = -advantage

        Actor[obs_start] += step_size_actor * grad_actor
        Critic[obs_start] -= step_size_critic * grad_critic
    ...
    np.save('critic_values.npy', Critic)
